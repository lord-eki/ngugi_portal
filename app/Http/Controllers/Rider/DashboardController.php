        ...
        $orders = Order::whereHas('delivery', $scope)
            ...
            ->through(fn (Order $order) => [
                ...
                'items'          => $order->orderItems->map(fn ($i) => [
                    'id' => $i->id, 'type' => $i->type, 'size' => $i->size,
                    'quantity' => (int) $i->quantity, 'is_bundled' => (bool) $i->is_bundled,
                    'bundle_quantity' => $i->bundle_quantity, 'bundle_size' => $i->bundle_size,
                    'amount' => (int) $i->amount, 'label' => $i->label,
                ]),
                ...
                'payment' => $order->payment ? [
                    'method' => $order->payment->method, 'method_label' => $order->payment->method_label,
                    'status' => $order->payment->status, 'phone' => $order->payment->phone,
                    'transaction_code' => $order->payment->transaction_code,
                ] : null,
            ]);

        return Inertia::render('rider/dashboard', ['stats' => $stats, 'orders' => $orders]);
    }
}